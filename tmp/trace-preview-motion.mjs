...
for (const delay of [0, 16, 40, 80, 120, 200, 320, 480, 640, 800, 1000]) {
  ...
  const sample = await page.evaluate(() => {
    const e=document.querySelector('[data-project-card][data-index="0"]'), g=document.querySelector('.project-preview-expand-ghost')
    ...
    return {t:performance.now(),scrollY,card:box(e),liveMedia:box(e?.querySelector('.project-media')),liveImage:box(li),liveImageStyle:ls?{width:ls.width,height:ls.height,left:ls.left,top:ls.top,right:ls.right,bottom:ls.bottom,transform:ls.transform,objectFit:ls.objectFit,objectPosition:ls.objectPosition}:null,ghost:box(g),ghostMedia:box(g?.querySelector('.project-media')),ghostImage:box(gi),ghostImageStyle:gs?{width:gs.width,height:gs.height,left:gs.left,top:gs.top,right:gs.right,bottom:gs.bottom,objectFit:gs.objectFit,objectPosition:gs.objectPosition}:null,ghostImageTransform:gs?gs.transform:null,ghostImageOrigin:gs?gs.transformOrigin:null,clip:g?getComputedStyle(g).clipPath:null,attrs:{ghosting:e?.dataset.projectPreviewExpandGhosting,ready:e?.dataset.projectPreviewReady}}
  })
