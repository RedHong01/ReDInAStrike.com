...
- On direct expansion, the card remains the document anchor while one fixed-geometry FLIP snapshot grows horizontally from its left or right card edge into the full-bleed preview over the shared ease-out curve; the copy enters after the surface has started expanding. Direct close uses the same horizontal edge track in reverse over the shared retract curve. Neither path uses the boundary dissolve or stacks a second reveal.
- About and Resume informational surfaces use the same document anchor and follow the live scroll progress. Scrolling down reveals their content from top to bottom; reversing direction reveals it from bottom to top, with the existing card order and geometry preserved.
- A second activation of an expanded project opens its detail body as an inline vertical drawer below the preview. It grows in the current scroll direction, keeps the preview card mounted as the anchor, and closes in reverse; it does not replace the home document with a new route render.
- Category filtering hands the outgoing full-bleed preview to one fixed-geometry snapshot that fades while the collapsed catalog continues its Fine Signal Snow transition. Preview-to-preview switching instead retracts that snapshot horizontally toward its original card edge while the incoming card opens from its own edge. The snapshot must not combine a center-origin rule scale with the edge clip, and rapid input must never leave more than one outgoing snapshot.
