...
Object.assign(caseStudyDetails, {
  "/monologue": {
    ...
    sections: [
      ...
      { kind: "source-breakdown", title: "Scene Before Chart", source: "FIGMA / TO BE CHOSEN NARRATIVE MATERIAL", lead: { en: "The source describes a multi-POV structure, but the player needs a scene before the chart. Enter the space, inspect a relic, trigger a perspective change, and use the new information to decide what to investigate next.", zh: "原稿描述了多視角結構，但玩家需要先看見一個場景。進入空間、調查敘事物件、觸發視角變化，再用新資訊決定下一步要調查什麼。" }, blocks: [ { title: "Scene", en: "The player controls a character and investigates an object rather than opening a narrative diagram.", zh: "玩家控制角色並調查物件，而不是先打開敘事圖表。" }, { title: "Perspective", en: "The discovery triggers another point of view. The purpose of the switch is to add information that was unavailable in the first view.", zh: "發現會觸發另一個視角。切換的目的，是加入第一個視角中沒有的資訊。" }, { title: "Next Choice", en: "The added information should change the next investigation or interpretation. A complete playable scene recording is still needed to verify that sequence.", zh: "新增資訊應該改變下一次調查或解讀。目前仍需要完整 playable scene 錄影來確認這段順序。" } ] },
  ...
  "/analog-game": {
    ...
    sections: [
      ...
      { kind: "source-breakdown", title: "Rules Before Board Image", source: "BUILD & SHOOT / GAME DESIGN DOCUMENT", lead: { en: "The board is a 15 × 15 floating island for up to four players. The design asks players to spend a turn on movement, building, inventory, or shooting, then read the physical change left for the next turn.", zh: "棋盤是一座 15 × 15 的浮空島，最多四名玩家。設計要求玩家在移動、建造、負重與射擊之間分配回合，再讀取留給下一回合的空間變化。" }, blocks: [ { title: "Movement Cards", en: "Movement is a card decision that changes position before the player commits to a build or shot.", zh: "移動卡在建造或射擊前先改變位置。" }, { title: "Territory Marks", en: "Territory marks make ownership affect later shots, so location becomes part of the attack rather than a backdrop.", zh: "領地標記讓擁有權影響之後的射擊，位置因此成為攻擊的一部分，而不是背景。" }, { title: "Inventory Weight", en: "Weight forces a choice between carrying more actions and keeping space for tactical cards.", zh: "負重迫使玩家在攜帶更多行動和保留戰術卡之間取捨。" }, { title: "Open Test", en: "The GDD records rule-memory and turn-length concerns. A recorded match is still needed to show how players explain a turn without a handoff.", zh: "GDD 記錄了規則記憶與回合過長的疑慮。仍需要一段對局錄影，展示玩家能否在沒有重新交接的情況下說出自己的回合。" } ] },
  ...
  "/bns_gdd": {
    ...
    sections: [
      { kind: "source-breakdown", title: "A Door Is a Rule", source: "CURTAIN / FIGMA GRID + UNIT + DOOR", lead: { en: "Curtain begins with a grid, units, and a door. “I’m so cold, may I come in?” turns the door from a graphic object into a boundary with a request attached to it.", zh: "Curtain 從格線、單位與門開始。「I’m so cold, may I come in?」讓門不只是圖形物件，而是帶著請求的邊界。" }, blocks: [ { title: "Grid", en: "Locate the unit and read the available space before the boundary changes.", zh: "先定位單位，再閱讀邊界變化前可用的空間。" }, { title: "Request", en: "The invitation gives the player a reason to read the door state rather than treating it as decoration.", zh: "邀請讓玩家有理由閱讀門的狀態，而不是把它當成裝飾。" }, { title: "Playable Gap", en: "The current source confirms the visual rule. A complete player recording is still needed to describe the final decision and result.", zh: "目前來源確認了視覺規則，但仍需要完整玩家錄影來描述最後的決定與結果。" } ] },
  ...
  "/alt-controller-2025-b": {
    ...
    sections: [
      ...
      { kind: "source-breakdown", title: "Gesture as Serving Sequence", source: "SUSHIGO / VISIONOS GESTURE REFERENCE", lead: { en: "The source sequence is pick up the knife, cut the fish, pick up the fish, combine it with rice, plate it, and deliver it. The gesture matters because each hand action prepares the next state of the dish.", zh: "原稿的流程是拿刀、切魚、拿魚、合米、裝盤與交付。手勢之所以重要，是因為每個手部動作都在準備料理的下一個狀態。" }, blocks: [ { title: "Grip", en: "Pick up the tool or ingredient. The object state should make the next action apparent.", zh: "拿起工具或食材，物件狀態應該提示下一步。" }, { title: "Pinch", en: "Use the cut or combine gesture to change the food state, then place it in the next workspace.", zh: "用切割或合併手勢改變食物狀態，再把它放入下一個工作區。" }, { title: "Delivery", en: "The final action is delivery. A complete 48-second pass is still the clearest evidence for the gesture-to-result relationship.", zh: "最後一步是交付。完整的 48 秒操作流程仍然是最清楚的手勢到結果證據。" } ] },
...
for (const [path, sections] of Object.entries(supportingReadableSections)) {
  if (caseStudyDetails[path]) caseStudyDetails[path].sections = sections
}
for (const [path, sections] of Object.entries(editorialReadableSections)) {
  const route = path === "untitled-sans" ? "/narrative-design-document" : `/${path}`
  const detail = framerProjectDetails[route]
  if (detail) detail.readableSections = sections
}
  ...
  return `
    ...
    <main class="site-main detail-page framer-derived-page" data-route="${escapeHtml(project.path)}">
      ...
      <article class="framer-derived-shell" aria-label="${escapeHtml(detail.title)} project page">
        ...
        ${detail.readableSections?.length ? `<section class="framer-derived-readable" aria-label="Readable design process">${detail.readableSections.map(section => `<section><h2>${escapeHtml(section.heading)}</h2><p>${escapeHtml(section.body)}</p></section>`).join("")}</section>` : ""}
...
function requestProjectDetailHeaderUpdate() {
  ...
  if (detailRow?.isConnected) {
    ...
    // Sticky positioning is clamped by the row's bottom edge. Without a
    // runway equal to the site-header seam, that clamp becomes
    // `drawerBottom` and releases the detail header while the final slice of
    // the article is still visible. Include the live header height in the
    // tail so the clamp remains at the sticky top until the drawer has fully
    // crossed the viewport; the row's matching negative margin keeps the
    // following catalogue content at the same document position.
    const stickyRunway = Math.max(0, headerHeight)
    const tailHeight = Math.max(0, openHeight - siblingHeight + stickyRunway)
  ...
  syncProjectDetailRule(drawerState, drawerRect, headerHeight)
  if (
...
function syncProjectDetailRule(state, drawerRect, headerBottom) {
  const { rule, card } = state
  if (!rule) return
  const edge = card.getClientRects()[0]?.bottom
  // Paint immediately outside the opaque lead, over the article itself.
  // A blend inside the card's isolated stacking context only inverts the
  // card background; this body-level line sees images and text too.
  const visible = Number.isFinite(edge) && edge >= headerBottom && edge < innerHeight &&
    drawerRect?.top <= edge + 1 && drawerRect.bottom > edge
  rule.hidden = !visible
  card.toggleAttribute("data-project-detail-invert-rule", visible)
  if (visible) rule.style.transform = `translateY(${Math.round(edge)}px)`
}
...
function createProjectPreviewExitGhost(card, { viewportPinned = false } = {}) {
  if (!card?.isConnected) return null
  const side = card.dataset.cardSide === "right" ? "right" : "left"
  ...
  if (snapshotImage && !viewportPinned) {
  ...
  ghost.style.setProperty("position", viewportPinned ? "fixed" : "absolute", "important")
  ...
  ghost.style.setProperty("--project-preview-ghost-left", `${sourceRect.left + (viewportPinned ? 0 : window.scrollX)}px`)
  ghost.style.setProperty("--project-preview-ghost-top", `${sourceRect.top + (viewportPinned ? 0 : window.scrollY)}px`)
...
function createProjectPreviewExpandGhost(card, sourceRect, targetRect, imageFrom, imageSourceState, onSettled) {
  ...
  let ghostScaleX = 1
  let ghostScaleY = 1
  let paintedImageTransform = { dx: 0, dy: 0, sx: 1, sy: 1 }
  const paintImageTransform = ({ dx, dy, sx, sy }) => {
    paintedImageTransform = {
      dx: Number(dx.toFixed(3)), dy: Number(dy.toFixed(3)),
      sx: Number(sx.toFixed(5)), sy: Number(sy.toFixed(5)),
    }
    const t = paintedImageTransform
    ghostImage.style.transform = `translate(${t.dx}px, ${t.dy}px) scale(${t.sx}, ${t.sy})`
  }
  if (ghostImage) {
    ghostImage.style.transformOrigin = "0 0"
    ghostImage.style.transform = "none"
  }

  let settled = false
  let settling = false
  let syncFrame = 0
  const imageSourceScrollY = window.scrollY
  const measureImageStartDescriptor = () => {
    if (!ghostImage || !imageFrom?.width || !imageFrom?.height) return null
    // Undo our known translation/scale mathematically. Removing the transform,
    // forcing layout, reading twice, and restoring it on every animation frame
    // made the measurement itself an additional rendering workload.
    const painted = ghostImage.getBoundingClientRect()
    const t = paintedImageTransform
    const imageBox = {
      left: painted.left - t.dx * ghostScaleX,
      top: painted.top - t.dy * ghostScaleY,
    ...
    // reported its natural dimensions yet, fall back to the element box; that
    // avoids the opposite failure mode where a missing bitmap measurement is
    // treated as a tiny rect and produces a 2x first-frame scale.
    if (imageSourceState?.bitmap && imageSourceState.visual?.width && targetImage?.width) {
      const scale = imageSourceState.visual.width / targetImage.width
      return {
        dx: (imageSourceState.visual.left - imageBox.left - (targetImage.left - imageBox.left) * scale) / ghostScaleX,
        dy: (imageSourceState.visual.top - imageBox.top - (targetImage.top - imageBox.top) * scale) / ghostScaleY,
        sx: scale,
        sy: scale,
      }
    }
    if (imageSourceState?.element) {
      const sourceBox = imageSourceState.element
      return {
        dx: (sourceBox.left - imageBox.left) / ghostScaleX,
        dy: (sourceBox.top - imageBox.top) / ghostScaleY,
    ...
    if (!targetImage?.width || !targetImage?.height) return null
    const scale = imageFrom.width / targetImage.width
    return {
      dx: (imageFrom.left - imageBox.left - (targetImage.left - imageBox.left) * scale) / ghostScaleX,
      dy: (imageFrom.top - imageBox.top - (targetImage.top - imageBox.top) * scale) / ghostScaleY,
      sx: scale,
      sy: scale,
    }
  }
  const updateImageMotion = (time) => {
    if (!imageMotionActive || !imageStartDescriptor || !ghostImage?.isConnected) return
    const raw = clamp((time - imageMotionStartedAt) / duration, 0, 1)
    const eased = smoothstep(raw)
    const current = measureImageStartDescriptor()
    // Keep the source anchor exact while the target geometry settles. The
    // start descriptor is recomputed against the current destination box,
    // then interpolated; a one-time FLIP would retain the old proxy width and
    // create the small final relocate the motion is meant to avoid.
    const descriptor = current || imageStartDescriptor
    // Source coordinates belong to the click's viewport. Keep them attached
    // to the document if the user scrolls during the reveal.
    const sourceScrollOffset = (imageSourceScrollY - window.scrollY) / ghostScaleY
    const dx = descriptor.dx * (1 - eased)
    const dy = (descriptor.dy + sourceScrollOffset) * (1 - eased)
  ...
  const syncGhostToLiveCard = () => {
    ...
    updateImageMotion(performance.now())
  ...
  syncGhostToLiveCard()
  if (ghostImage) {
    // Resolve the destination fit only after the snapshot is attached. The
    // expanded selectors (including the desktop contain rule) are scoped to
    // the document tree; reading a detached clone would report the compact
    // source fit and skip the cover→contain handoff branch.
    targetImageFit = getComputedStyle(ghostImage).objectFit.trim().toLowerCase()
    targetImagePosition = getComputedStyle(ghostImage).objectPosition.trim() || "50% 50%"
    imageFitChanges = Boolean(
      imageSourceState?.element &&
      sourceImageFit &&
      targetImageFit &&
  ...
  if (ghostImage && imageFrom?.width > 0 && imageFrom?.height > 0) {
    imageStartDescriptor = measureImageStartDescriptor()
  ...
  window.requestAnimationFrame(() => {
    ...
    if (imageStartDescriptor) {
      // Start the image motion on the same clock as the clip transition. The
      // descriptor is refreshed by `updateImageMotion` on every frame, so a
      // settling content-edge proxy cannot introduce a final one-frame snap.
      imageMotionStartedAt = performance.now()
      imageMotionActive = true
      updateImageMotion(imageMotionStartedAt)
    }
    if (copy) {
...
function projectPreviewImageRect(card) {
  ...
  return projectPreviewBitmapRect(image, image.getBoundingClientRect())
}

function projectPreviewBitmapRect(image, rect, style = getComputedStyle(image)) {
  ...
  if (!contain || !image.naturalWidth || !image.naturalHeight) return {
...
function projectPreviewImageState(card) {
  ...
  const bitmap = style.objectFit.trim().toLowerCase() === "contain" && image.naturalWidth > 0 && image.naturalHeight > 0
  const element = projectPreviewRect(image.getBoundingClientRect())
  return {
    visual: element ? projectPreviewBitmapRect(image, element, style) : null,
    element,
...
function applyProjectPreviewExitTarget(exitMotion, targetCard) {
  ...
  if (exitMotion.viewportPinned) {
    // The drawer's sticky lead has no nearby compact destination after its
    // article is removed. Retract its captured surface to its original edge;
    // following the distant grid cell would turn this into a vertical wipe.
    const right = ghost.dataset.projectPreviewMotionSide === "right"
    ghost.style.setProperty("--project-preview-exit-left", `${right ? sourceRect.width : 0}px`)
    ghost.style.setProperty("--project-preview-exit-right", `${right ? 0 : sourceRect.width}px`)
    ghost.style.setProperty("--project-preview-exit-top", "0px")
    ghost.style.setProperty("--project-preview-exit-bottom", "0px")
    ghost.dataset.projectPreviewExitMode = "edge"
...
function runProjectPreviewExitGhost(
  exitMotion,
  targetCard,
  { clearTransition = false, motionId = 0, fade = false, onCleaned = null } = {},
) {
  ...
  if (!fade && !exitMotion.viewportPinned && exitImage && targetImage) {
  ...
  const cleanup = () => {
    ...
    ghost.__projectPreviewExitImageCleanup = null
    releaseProjectPreviewExitSource(ghost)
...
function setProjectPreview(card, expanded) {
  ...
  if (drawer && (!expanded || drawer.card !== card)) {
    beginProjectDetailMotion()
    cancelSectionScroll()
    closeProjectDetailDrawer({ immediate: true, refresh: false })
  }
  if (!expanded) {
    ...
    // Keep the expanded surface as a paint-only snapshot while the live card
    // returns to its compact grid cell. Animating the live card directly lets
    // its grid tracks recalculate on the same frame as the final WAAPI tick;
    // the ensuing class commit then exposes a second, compact geometry and
    // reads as a one-frame refresh. The exit ghost owns that entire retract,
    // just as the expand ghost owns the opening wipe.
    const exitMotion = createProjectPreviewExitGhost(card)
    if (!exitMotion) { finish(); return }
    document.documentElement.dataset.projectPreviewTransition = "exiting"
    // Commit the compact layout under the snapshot so the target rect is
    // measured in the real row. The source row/card remain hidden by the
    // exit-ghost ownership classes until the retract has painted its final
    // frame.
    commitProjectPreviewState(card, false, { refresh: false })
    runProjectPreviewExitGhost(exitMotion, card, {
      clearTransition: true,
      motionId,
      onCleaned: () => {
        clearProjectPreviewExpandGhostState(card)
  ...
  if (switchAnchor) restoreProjectDetailSwitchAnchor(switchAnchor, { flow: true })
  if (drawer) requestAnimationFrame(() => requestAnimationFrame(() => {
    if (motionId === siteState.projectPreviewMotionId) endProjectDetailMotion()
  }))
...
function closeProjectDetailDrawer({ immediate = false, afterClose = null, refresh = true } = {}) {
...
function restoreProjectDetailSwitchAnchor(anchor, { flow = false } = {}) {
  if (!anchor?.element?.isConnected) return
  const row = flow ? anchor.element.closest(".project-row") : null
  const position = row?.style.getPropertyValue("position")
  const priority = row?.style.getPropertyPriority("position")
  const top = row?.style.getPropertyValue("top")
  const topPriority = row?.style.getPropertyPriority("top")
  // Sticky rects report the viewport pin, not the document position that
  // changed when the article was removed. Measure normal flow without ever
  // painting the temporary positioning mode.
  row?.style.setProperty("position", "relative", "important")
  row?.style.setProperty("top", "auto", "important")
  const rect = anchor.element.getClientRects()[0]
  const destination = rect ? window.scrollY + rect.top - anchor.top : NaN
  if (row) {
    if (position) row.style.setProperty("position", position, priority)
    else row.style.removeProperty("position")
    if (top) row.style.setProperty("top", top, topPriority)
    else row.style.removeProperty("top")
  }
  if (!Number.isFinite(destination)) return
  window.scrollTo({
    top: clamp(destination, 0, pageMaxScrollY()),
    left: 0,
    behavior: "instant",
  })
  // Structural compensation is not a user scroll gesture. Rebase both
  // scroll owners so it cannot reverse the header, dither, or magnet motion.
  siteState.lastScrollY = window.scrollY
  siteState.pendingScrollDelta = 0
  window.__RED_SCROLL_FRAME__?.rebase?.()
  syncScrollDrivenVisuals({ syncHeader: false, publishMoving: false })
  requestProjectDetailHeaderUpdate()
}
...
function openProjectDetailDrawer(card, target) {
  ...
  const resizeObserver = typeof ResizeObserver === "function" && inner
    ? new ResizeObserver(() => {
      const state = activeProjectDetailDrawer()
      if (state?.element !== drawer || state.resizeFrame) return
      // Observing the catalogue and rewriting its sticky runway inside the
      // same ResizeObserver delivery creates a feedback loop in WebKit.
      // Coalesce notifications; regular scroll still updates synchronously.
  ...
  rule.className = "project-detail-invert-rule"
  rule.setAttribute("aria-hidden", "true")
  rule.hidden = true
  document.body.appendChild(rule)
  const drawerState = { element: drawer, card, row, stickyTail, resizeObserver, rule }
...
function interruptProjectProgrammaticScroll() {
...
window.addEventListener("touchstart", interruptProjectProgrammaticScroll, { passive: true })
