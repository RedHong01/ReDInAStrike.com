...
function beginDirectTransition(state, targetBits) {
  ...
  // Fine Signal already returned to the captured boundary-clipped surface.
  // This canvas is now only an ownership guard while the canonical viewport
  // field is prepared underneath it; synchronizing the target must not start
  // a second randomized pixel transition.
  drawBinaryBits(
    state.ctx,
    state.imageData,
    state.framePixels,
  ...
  state.phase = "viewport-direct-sync"
  finishState(state)
}
