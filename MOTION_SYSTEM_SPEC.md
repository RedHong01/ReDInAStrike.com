...
- On direct expansion, the card remains the document anchor while one fixed-geometry FLIP snapshot grows horizontally from its left or right card edge into the full-bleed preview over the shared ease-out curve; the copy enters after the surface has started expanding. Direct close keeps the existing 420ms geometric retraction. Neither path uses the boundary dissolve or stacks a second reveal.
